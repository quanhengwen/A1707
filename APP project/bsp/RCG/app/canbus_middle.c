...
/***************************************************************************************************
*\Function      CAN_Read_CH2_DutyCycle_D
*\Description   
*\Parameter     buf
*\Parameter     len
*\Return        u32
*\Note
*\Log           2018.10.17    Ver 1.0    baron
*               ?????
***************************************************************************************************/
u32 CAN_Read_CH2_DutyCycle_D(u8* buf, u32 len)
{
	u32 data_len = 8;
	u32 i = 0;
	...
	buf[i++] = 0x01;//(u8)iap_param.ID;//ID
	buf[i++] = 0x02;//(u8)(iap_param.ID >> 8);//ID
	buf[i++] = 0x01;
	buf[i++] = (u8)CH2_DutyCycle_D;//低位在前
	buf[i++] = (u8)(CH2_DutyCycle_D >> 8);//高位在后
	if(out_control.out_control_flag == 0)//在有效信号内
	{
	  buf[i++] = 0x00;
	}
	else
	...
	buf[i++] = 0x00;
	buf[i++] = Get_Sum8(&buf[0], data_len - 1);
	return i;
}
/***************************************************************************************************
*\Function      CAN_Read_CH2_DutyCycle_R
*\Description   
*\Parameter     buf
*\Parameter     len
*\Return        u32
...
*               ?????
***************************************************************************************************/
u32 CAN_Read_CH2_DutyCycle_R(u8* buf, u32 len)
{
	u32 data_len = 8;
	u32 i = 0;
	...
	buf[i++] = 0x02;//(u8)(iap_param.ID >> 8);//ID
	buf[i++] = 0x02;
	buf[i++] = (u8)CH2_DutyCycle_R;//低位在前
	buf[i++] = (u8)(CH2_DutyCycle_R >> 8);//高位在后
	if(out_control.out_control_flag == 0)//在有效信号内
	{
	  buf[i++] = 0x00;
	}
...
/***************************************************************************************************
*\Function      CAN_Read_CH2_DutyCycle_N
*\Description   
*\Parameter     buf
*\Parameter     len
*\Return        u32
...
*               ?????
***************************************************************************************************/
u32 CAN_Read_CH2_DutyCycle_N(u8* buf, u32 len)
{
	u32 data_len = 8;
	u32 i = 0;
...
void CAN_Heartbeat_Package_Send(void)
{
	...
	buf[i++] = soft_switch;//软开关工作模式//(u8)realtime_param.voltage;//电压低
	buf[i++] = dev_param.time_interval; //(u8)(realtime_param.voltage >> 8);//电压高
...
void CAN_Paramset_Receive_Analyse(u8* buf)//需要根据自己的协议解析函数
{
	...
	dev_param.time_interval = buf[26];//上传速度
}
/***************************************************************************************************
...
u32 CAN_Paramset_Read_Construct(u8* buf , u32 len)//需要每人根据自己的项目组包
{
	...
	buf[i++] = dev_param.time_interval;//速度上传间隔
...
u32 CAN_Automatic_Check_Construct(u8* buf, u32 len)//根据不同的设备进行组包
{
	...
	buf[i++] = soft_switch;//软开关工作模式     //具体需检测参数单独添加
	buf[i++] = (u8)realtime_param.current;// 电流低
	buf[i++] = (u8)(realtime_param.current >> 8);// 电流高
	buf[i++] = 0x01;//can通信正常
	buf[i++] = Get_Sum8(buf + 1, data_len - 3);
