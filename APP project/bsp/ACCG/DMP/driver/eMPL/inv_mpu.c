...
const struct test_s test = {
    ...
    .reg_gyro_fsr   = 0,    /* 250dps. */
    .reg_accel_fsr  = 0x0,  /* Accel FSR setting = 2g. 加速度的量程*/
...
//返回值:0,正常
//    其他,失败
uint8_t mpu_mpl_get_data(float *pitch,float *roll,float *yaw,mems_original_data_type *mems_data)
{
    ...
    *yaw   = -data[2] / q16;
	...
	memcpy(&mems_data->gyro_data.x_axis_data, gyro, 6); 
	memcpy(&mems_data->acc_data.x_axis_data, accel_short, 6); 
	memcpy(&mems_data->mag_data.x_axis_data, compass_short, 6); 
