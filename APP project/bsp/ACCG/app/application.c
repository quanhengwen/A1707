...
//        rt_thread_startup(tid); 
	...
	tid = rt_thread_create("imuT", IMU_thread_entry, RT_NULL, 1024*4, 6, 5);
    if (tid != RT_NULL)
        rt_thread_startup(tid); 
	
	tid = rt_thread_create("cuT", max9611_thread_entry, RT_NULL, 1024, 8, 5);
	if (tid != RT_NULL)
		rt_thread_startup(tid); 
	
	tid = rt_thread_create("reportT", report_thread_entry, RT_NULL, 1024, 10, 5);
	if (tid != RT_NULL)
		rt_thread_startup(tid); 
	
	tid = rt_thread_create("motT", motor_ctrl_thread_entry, RT_NULL, 1024, 4, 5);
    if (tid != RT_NULL)
        rt_thread_startup(tid);
	
	tid = rt_thread_create("hallT", hall_detect_thread_entry, RT_NULL, 1024, 9, 5);
    if (tid != RT_NULL)
        rt_thread_startup(tid);
