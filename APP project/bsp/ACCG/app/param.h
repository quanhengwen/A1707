...
typedef struct dev_param_type_st
{
	...
	u16 pitch_threshold_value;//俯仰角报警阈值  单位是0.01度,即角度*100
	u16 roll_threshold_value;//横滚角报警阈值   单位是0.01度,即角度*100
	u16 yaw_threshold_value;//偏航角报警阈值    单位是0.01度,即角度*100 
	u16 stepper_motor_distance;//步进电机固定执行的距离，单位mm
}dev_param_type;
#pragma pack()
...
typedef struct realtime_param_type_st
{
	...
	u16 pitch;//俯仰角  单位是0.01度,即角度*100
	u16 roll;//横滚角  单位是0.01度
	u16 yaw;//偏航角  单位是0.01度
	u16 acc_x;
	u16 acc_y;
	u16 acc_z;
}realtime_param_type;
