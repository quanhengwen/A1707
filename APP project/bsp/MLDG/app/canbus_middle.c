...
void CAN_Heartbeat_Package_Send(void)
{
	...
	buf[i++] = mode;//闪烁模式(u8)realtime_param.voltage;//电压低
	buf[i++] = 0x00;//(u8)(realtime_param.voltage >> 8);//电压高
...
void CAN_Paramset_Receive_Analyse(u8* buf)//需要根据自己的协议解析函数
{
	...
	switch(buf[26])//装饰灯的闪烁模式选择
	{
	  case 0x00: Mldg_ModeOff(3);
		break;
		case 0x01: Mldg_Mode1(3);
		break;
		case 0x02: Mldg_Mode2(3);
		break;
		case 0x03: Mldg_Mode3(3);
		break;
		default: break;
	}
}
...
u32 CAN_Automatic_Check_Construct(u8* buf, u32 len)//根据不同的设备进行组包
{
	...
	buf[i++] = mode;//装饰灯闪烁模式     //具体需检测参数单独添加
	buf[i++] = (u8)realtime_param.current;// 电流低
	buf[i++] = (u8)(realtime_param.current >> 8);// 电流高
	buf[i++] = 0x01;//can通信正常
	buf[i++] = 0x00;
