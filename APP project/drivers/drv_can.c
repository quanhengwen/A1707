    ...
    if(CAN_GetITStatus(can_dev->can, it_status) != RESET)
    {
        ...
        while (CAN_MessagePending(can_dev->can, can_fifo) > 0)
        {
            ...
            CAN_Receive(can_dev->can, can_fifo, &(can_dev->CANRxBuf[can_dev->CanRx_Write++]));
					...
					#ifdef DCG_board
					  if(can_dev->CANRxBuf[can_dev->CanRx_Write-1].StdId==0x100)
						{
						//	GPIO_SetBits(GPIOC, GPIO_Pin_1);
						  can_dev->CanRx_Write--;
							if(can_dev->CANRxBuf[can_dev->CanRx_Write].Data[0]==(u8)iap_param.ID&&can_dev->CANRxBuf[can_dev->CanRx_Write].Data[1]==(iap_param.ID >> 8)) //ID判断 是发给自己的
								{
                  Motor_time_err=0;  //清零
									if(can_dev->CANRxBuf[can_dev->CanRx_Write].Data[2]==0x01) //前进    速度是 0~35
									{
										if( Motor_state==1||Motor_state==0)
										{
											Motor_cotorl=1;
											run_motor; run_zheng ; 	
											//if(can_dev->CANRxBuf[can_dev->CanRx_Write].Data[3]>20) can_dev->CANRxBuf[can_dev->CanRx_Write].Data[3]=20;
											CCR_Va0=(u16) (can_dev->CANRxBuf[can_dev->CanRx_Write].Data[3]/35.0*999.0);
											PWM_out(CCR_Va0);
											Motor_state=1;
								...
								if(can_dev->CANRxBuf[can_dev->CanRx_Write].Data[2]==0x02) //后退
								{  
								...
								if(can_dev->CANRxBuf[can_dev->CanRx_Write].Data[2]==0x00) //缓慢停车
								{	
										Motor_cotorl=0;	
										rt_mb_send(do_motor_sem,Stop_low);  // 发送停车信号 
								}
								
								if(can_dev->CANRxBuf[can_dev->CanRx_Write].Data[2]==0x03) //紧急刹车
								{	
										Motor_cotorl=3;	
										Stop_fast0();//rt_mb_send(do_motor_sem,Stop_fast);  // 发送停车信号 
								}
...
//								rt_kprintf("LEN:%d\n", can_dev->CANRxBuf[can_dev->CanRx_Write].DLC);
//								for(i = 0; i<can_dev->CANRxBuf[can_dev->CanRx_Write].DLC;i++)
//								{
//									rt_kprintf("%02X ", can_dev->CANRxBuf[can_dev->CanRx_Write].Data[i]);
//								}
//								rt_kprintf("\n");
