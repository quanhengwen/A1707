...
void CAN_Heartbeat_Package_Send(void)
{
	...
	buf[i++] = (u8)realtime_param.voltage;//电池电压低
	buf[i++] = (u8)(realtime_param.voltage >> 8);//电池电压高
	buf[i++] = state.work_mode;//电池及充放电模式
	buf[i++] = 0x00;          //无用
...
u32 CAN_Paramset_Read_Construct(u8* buf , u32 len)//需要每人根据自己的项目组包
{
	...
	buf[i++] = state.switch_chn1;//通道1开关
	buf[i++] = state.switch_chn2;//通道2开关
	buf[i++] = state.switch_chn3;//通道3开关
	buf[i++] = state.switch_chn4;//通道4开关
	buf[i++] = (u8)max9611_data[0].Current;              //四路通道电流
	buf[i++] = (u8)((u8)max9611_data[0].Current >> 8);
	buf[i++] = (u8)max9611_data[1].Current;
	buf[i++] = (u8)((u8)max9611_data[1].Current >> 8);	
	buf[i++] = (u8)max9611_data[2].Current;
	buf[i++] = (u8)((u8)max9611_data[2].Current >> 8);
	buf[i++] = (u8)max9611_data[3].Current;
	buf[i++] = (u8)((u8)max9611_data[3].Current >> 8);
  buf[i++] = (u8)max9611_data_charge.Current;
  buf[i++] = (u8)((u8)max9611_data_charge.Current >> 8);
	buf[i++] = 0x00;//非关机运行	
...
void CAN_Paramset_Receive_Analyse(u8* buf)//需要根据自己的协议解析函数
{
	...
	if(buf[26] == 1){ ON_36V01; state.switch_chn1 = 1;}//通道1处理
	else { OFF_36V01; state.switch_chn1 = 0;}
	if(buf[27] == 1){ ON_36V02; state.switch_chn2 = 1;}//通道2处理
	else { OFF_36V02; state.switch_chn2 = 0;}
	if(buf[28] == 1){ ON_36V03; state.switch_chn3 = 1;}//通道3处理
	else { OFF_36V03; state.switch_chn3 = 0;}
	if(buf[29] == 1){ ON_36V04; state.switch_chn4 = 1;}//通道4处理
	else { OFF_36V04; state.switch_chn4 = 0;}
	//关机
	if(buf[38] == 1) 
	{
...
u32 CAN_Automatic_Check_Construct(u8* buf, u32 len)//根据不同的设备进行组包
{
	...
	buf[i++] = state.work_mode;//软开关工作模式     //具体需检测参数单独添加
	buf[i++] = 0x01;//can通信正常
	buf[i++] = (u8)max9611_data[0].Current;              //四路通道电流
	buf[i++] = (u8)((u8)max9611_data[0].Current >> 8);
	buf[i++] = (u8)max9611_data[1].Current;
